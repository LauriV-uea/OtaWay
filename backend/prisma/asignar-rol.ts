 ...
 *   yarn asignar-rol --email gestor@otaway.local     --rol gestor-de-cancha --sede <venueId>
 *
 ...
 *
 * El usuario debe existir ya en OtaWay, y solo existe despues de su PRIMER LOGIN
...
const main = async (): Promise<void> => {
	...
	if (!user) {
		throw new Error(
			`No existe el usuario ${args.email} en OtaWay. Debe iniciar sesion al menos una vez: el callback lo crea automaticamente.`,
		);
	}
