...
- [x] Agregar el cliente `otaway-registration-service` a `keycloak/realm-otaway.json` (`serviceAccountsEnabled: true`, sin flujos de usuario, rol de cliente `manage-users` de `realm-management` en su cuenta de servicio).
...
- [x] Crear `database/organizaciones-sedes/access-request.db.ts` (`getAccessRequests`, `approveAccessRequest`, `rejectAccessRequest`). **Cambio de diseño respecto al plan**: en vez de `createAccessRequest` separado, se implementó `createUserWithAccessRequest(keycloakId, user, organization, venue)`, que crea User+Profile+AuthAccount **y** Organization+Venue+AccessRequest en **una sola transacción**. Se detectó en pruebas manuales que crear el usuario y la solicitud en transacciones separadas dejaba usuarios huérfanos en OtaWay si la segunda fallaba después de que la primera ya hubiera confirmado.
- [x] Crear `services/auth/register.service.ts` (orquestación completa + compensación ante fallo de OtaWay tras éxito en Keycloak).
