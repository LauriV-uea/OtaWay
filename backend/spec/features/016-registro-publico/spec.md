...
2. Crea la identidad real en **Keycloak** (no en un password local de OtaWay) usando la API de administración de Keycloak — la contraseña nunca se guarda en la base de OtaWay (`User.passwordHash` sigue sin usarse, igual que hoy).
3. Crea el `User`/`UserProfile`/`AuthAccount` en OtaWay, igual que ya hace `findOrSyncByOAuth`, pero de forma síncrona en el mismo request (no en el primer login posterior).
4. Según `accountType`:
...
```ts
{
  email: string;              // único en Keycloak y en OtaWay
...
- `409` — email ya existe en Keycloak o en OtaWay.
...
- `500` — fallo al crear en Keycloak o en OtaWay (ver "Consistencia" abajo).
...
**Registro — Futbolista**
- Un registro válido con `accountType: 'futbolista'` crea exactamente un usuario en Keycloak y exactamente un `User`/`UserProfile`/`AuthAccount` en OtaWay, con el rol global `Futbolista` ya asignado.
...
- Un registro válido con `accountType: 'gestor-de-cancha'` crea el usuario (Keycloak + OtaWay), la `Organization` y al menos una `Venue` en `status: 'PENDING_APPROVAL'`, y una fila en `OrganizationAccessRequest` con `status: 'PENDING'` — pero **ningún** `UserRole` para ese usuario.
...
- La contraseña nunca se registra en logs, nunca se guarda en la base de OtaWay, y viaja solo hacia Keycloak (HTTPS en cualquier entorno real).
- El email se valida como único tanto en Keycloak como en OtaWay antes de crear nada; un duplicado en cualquiera de los dos responde `409` sin crear una cuenta a medias.
