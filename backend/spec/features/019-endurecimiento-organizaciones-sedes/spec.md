...
Cierra siete gaps reales de seguridad y robustez encontrados en el CRUD de `Organization`/`Venue` (feature `004`, hecho ✅ desde 2026-06-30) al inspeccionarlo para la nueva pantalla administrativa del frontend (`otaway-ionic`, feature `010-gestion-organizaciones-sedes`). Aplica al módulo `organizaciones-sedes` exactamente el mismo patrón de robustez ya validado en producción por la feature `018` (roles): guardia de alcance por organización, concurrencia optimista, transacciones con auditoría, y schemas Zod estrictos.
...
- [ ] Los permisos sembrados `sedes.read`/`sedes.manage` (código muerto hoy, documentado en `api-integration.md` de `otaway-ionic`) no se activan ni se usan en ningún endpoint como parte de esta feature — se deja explícitamente fuera de alcance (ver abajo) para no ampliar el cambio más allá de lo pedido.
- [ ] `otaway-ionic` (feature `010`) puede actualizar su documentación de dependencia de backend a "resuelto" una vez esta feature esté hecha — no se requiere ningún cambio adicional de contrato más allá de `expectedUpdatedAt` (nuevo campo requerido en `PATCH`) y `venuesCount` (nuevo campo opcional en el listado).
...
- **Cambiar el envelope no estándar `{organizations, meta}`/`{venues, meta}`** a `{data, meta}`. Es una decisión de compatibilidad con el frontend ya consumido; cambiarlo rompería `otaway-ionic` sin necesidad — no forma parte de este endurecimiento.
