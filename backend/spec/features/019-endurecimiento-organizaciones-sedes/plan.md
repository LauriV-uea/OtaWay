...
- **No se activan los permisos `sedes.read`/`sedes.manage`.** Cambiarlos activaría un contrato de autorización distinto al que ya consume `otaway-ionic` (que usa `organizaciones.*` para todo, ver `api-integration.md`); es una decisión de producto aparte, no un endurecimiento de seguridad.
