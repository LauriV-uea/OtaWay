...
- [x] Releer `otaway-ionic/src/features/admin/navigation/admin-navigation.ts` en el momento de implementar.
...
- [ ] Si la sección nueva tiene una entrada real en la navegación de algún cliente (p. ej. `otaway-ionic`), añadir el menú correspondiente al arreglo `MENUS` de `prisma/seed.ts` con sus permisos reales asociados — nunca inventar la asociación antes de que la ruta y el permiso existan de verdad en el backend.
