...
Mientras tanto, `otaway-ionic` (el único consumidor real hoy) no pregunta nada a `otaway` sobre menús: su navegación de administración vive hardcodeada en `otaway-ionic/src/features/admin/navigation/admin-navigation.ts` como un arreglo estático (`ADMIN_NAVIGATION`), con `requiredPermissions` escritos a mano por cada ítem. Esto es un hallazgo verificado, no una suposición: se confirmó por lectura directa de ese archivo y por búsqueda exhaustiva de `menu`/`/api/menus` en `otaway-ionic/src`, sin resultados de consumo. El resultado es que las tablas `menus`/`menu_permissions` están vacías por diseño incompleto, no por un bug puntual — cualquier superficie (Ionic, Swagger, un futuro cliente) que intente leer menús reales desde `otaway` encuentra necesariamente cero filas, porque nunca se sembraron y no existe endpoint para poblarlas ni consultarlas.
...
### Consumidor real (`otaway-ionic`) — navegación estática, no conectada a `Menu`

- `otaway-ionic/src/features/admin/navigation/admin-navigation.ts` define `ADMIN_NAVIGATION` como un arreglo TypeScript estático: 4 ítems (`users`, `roles`, `permissions`, `organizations`), cada uno con `id`, `label`, `description`, `icon`, `path` y `requiredPermissions` escritos a mano en el propio archivo.
- Búsqueda de `/api/menus` y de la palabra `menu` en `otaway-ionic/src`: sin ningún consumo de un endpoint de menús. La navegación de administración de Ionic hoy no depende ni sabe de las tablas `menus`/`menu_permissions` de `otaway`.
- Esto es información de contexto, no un compromiso de esta feature: `otaway-ionic` es otro repositorio con su propia constitución y su propio flujo SDD. Esta spec no ordena ni asume cambios ahí (ver "Fuera de alcance"). Se documenta porque explica, con evidencia, por qué las tablas de menús aparecen vacías: nunca se sembraron y nada las pide.
...
- Existe un catálogo de menús persistido, sembrado de forma idempotente (mismo patrón `findUnique`-antes-de-`create` por `code` único que ya usa `prisma/seed.ts` para permisos), reflejando **exactamente** las secciones administrables reales y ya existentes hoy en el único consumidor conocido (`otaway-ionic`): los 4 ítems de `ADMIN_NAVIGATION` (`users`, `roles`, `permissions`, `organizations`), sin inventar secciones nuevas que no existan en ningún cliente real.
...
- Cualquier cambio en `otaway-ionic`: migrar `ADMIN_NAVIGATION` a consumir `GET /api/menus`, agregar estados de carga/vacío/error en su UI, o cualquier otro ajuste de interfaz. `otaway` es un backend puro sin frontend (`AGENTS.md` §1, §11); ese trabajo, si se decide hacerlo, es una feature separada en el repositorio `otaway-ionic` bajo su propio flujo SDD, y debe partir de los contratos reales que esta feature deja documentados — no se asume ni se diseña aquí.
