...
El schema ya distingue dos conceptos, y el dominio de OtaWay los usa así:
...
1. `docker compose up -d` deja un Keycloak accesible con el realm `otaway` y el cliente `otaway-api` configurado con PKCE `S256` obligatorio.
...
4. `GET /api/auth/login` en el navegador redirige a Keycloak; tras autenticarse, el callback valida `state`, `nonce`, firma, issuer y audiencia, crea el usuario en OtaWay vía `findOrSyncByOAuth` y deja una cookie de sesión cifrada.
...
- **Colisión de email en el primer login.** `findOrSyncByOAuth` enlaza al usuario por `authAccount(provider, sub)`. Un usuario creado antes vía `POST /api/users` no tiene `authAccount`, así que en su primer login el callback intenta crear otro usuario con el mismo email y falla por unicidad (P2002). El orden correcto es: crear el usuario **en el IdP** → iniciar sesión → OtaWay lo crea solo → asignar rol.
