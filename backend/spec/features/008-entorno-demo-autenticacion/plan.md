...
El realm se declara en `keycloak/realm-otaway.json` y Keycloak lo importa al arrancar. Así el entorno es reproducible por cualquier alumno con un solo comando y queda versionado (la constitución prohíbe versionar secretos reales, pero éste es un IdP de desarrollo local con credenciales de juguete).
...
| Variable | Valor |
|---|---|
| `OAUTH_ISSUER` | `http://localhost:8080/realms/otaway` (sin barra final: es el `iss` exacto que emite Keycloak) |
...
4. Login en el navegador → OtaWay crea el usuario solo.
