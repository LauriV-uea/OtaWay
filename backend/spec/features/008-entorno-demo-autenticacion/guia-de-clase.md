# Guía de clase — Autenticación y Autorización en OtaWay
...
OtaWay **nunca ve la contraseña**. No existe un endpoint que reciba usuario y clave. El backend delega la identidad a un **Identity Provider** (Keycloak) mediante OAuth 2.0 Authorization Code + PKCE:

```
Navegador          OtaWay (backend)              Keycloak (IdP)
    │                     │                             │
    │  GET /api/auth/login│                             │
    ├────────────────────>│  genera state + PKCE        │
    ...
    │<────────302─────────┤                             │
    │                                                   │
    │  el usuario tipea su clave AQUÍ, no en OtaWay   │
    ├──────────────────────────────────────────────────>│
    │<─────────────302 con ?code=… &state=… ────────────┤
    │                     │                             │
...
La contraseña viaja al IdP. OtaWay sólo recibe un **código de un solo uso** y lo canjea por tokens.
...
Usuarios ya cargados en Keycloak (contraseña `otaway123` en los tres):

| Usuario | Email | Para el rol |
|---|---|---|
| `futbolista` | futbolista@otaway.local | Futbolista |
| `gestor` | gestor@otaway.local | Gestor de Cancha |
| `administrador` | administrador@otaway.local | Administrador |
...
Redirige a Keycloak → el alumno se autentica como `futbolista` / `otaway123` → Keycloak devuelve al callback → OtaWay deja la cookie de sesión y muestra la sesión.
...
- La cookie final `otaway_session` sale con `HttpOnly; Secure; SameSite=Lax`. `HttpOnly` significa que **JavaScript no puede leerla**: ésa es la defensa contra XSS.

El usuario **no existía** en la base de OtaWay: lo creó el callback en su primer login (`findOrSyncByOAuth`), enlazándolo al IdP por `authAccount(provider, sub)`.
...
```bash
curl -i http://localhost:3000/api/auth/session -H "Cookie: otaway_session=<valor>"
```
...
yarn asignar-rol --email gestor@otaway.local --rol gestor-de-cancha --sede 9d2a6897-3c6d-4ccd-8e7f-1bbdc5e01605
```
...
```bash
curl -i -X POST http://localhost:3000/api/auth/logout -H "Cookie: otaway_session=<valor>"
```

```
HTTP/1.1 204 No Content
Set-Cookie: otaway_session=; Max-Age=0; Expires=Thu, 01 Jan 1970 00:00:00 GMT; Path=/
```
