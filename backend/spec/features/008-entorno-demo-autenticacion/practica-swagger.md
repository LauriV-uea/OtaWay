...
Credenciales de práctica: usuario `futbolista`, contraseña `otaway123`.
...
Con **DevTools → Network** abierto y la casilla *Preserve log* activada. Autenticarse como `futbolista` / `otaway123`.
...
2. **La pantalla de Keycloak.** La contraseña se escribe **aquí**, en el Identity Provider. OtaWay **nunca la ve**. No existe ningún endpoint en esta API que reciba una contraseña — pueden buscarlo en la lista de Swagger.

3. **La vuelta a `/api/auth/callback?code=…&state=…`**. OtaWay recibe un **código de un solo uso**, no la contraseña. Y responde con:

```
Set-Cookie: otaway_session=Fe26.2**…; HttpOnly; Secure; SameSite=Lax; Path=/
```
...
> Este usuario **no existía** en la base de datos de OtaWay hace un minuto. ¿Quién lo creó?
...
```
Set-Cookie: otaway_session=; Max-Age=0; Expires=Thu, 01 Jan 1970 00:00:00 GMT
```
...
Repitan el ejercicio, pero **antes** de cerrar sesión copien el valor de la cookie (DevTools → Application → Cookies → `otaway_session`).

Cierren sesión (`POST /auth/logout` → `204`). Y ahora, desde una terminal, reenvíen a mano esa cookie:

```bash
curl -i http://localhost:3000/api/auth/session \
  -H "Cookie: otaway_session=<el-valor-que-copiaron>"
```
