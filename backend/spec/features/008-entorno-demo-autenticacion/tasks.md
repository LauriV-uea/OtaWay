...
- [x] `keycloak/realm-otaway.json`: realm `otaway`, cliente `otaway-api` (confidencial, PKCE S256, redirect a `/api/auth/callback`), usuarios `futbolista`, `gestor`, `administrador`.
- [x] Verificado el discovery: issuer `http://localhost:8081/realms/otaway`, PKCE `S256` soportado.
...
- [x] Callback → **302**, cookie `otaway_session` con `HttpOnly; Secure; SameSite=Lax`; usuario creado solo en OtaWay.
