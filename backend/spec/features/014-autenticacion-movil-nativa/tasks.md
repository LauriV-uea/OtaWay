...
- [x] `keycloak/realm-otaway.json`: `otaway-mobile.directAccessGrantsEnabled = true`.
...
- [x] `POST /api/auth/mobile/login` con `futbolista`/`otaway123` real → `sessionToken` válido, sin ningún navegador.
...
- [x] Flujo de cookie del cliente web (`otaway-api`) sigue funcionando sin cambios (regresión).
