...
1. **`keycloak/realm-otaway.json`** — `otaway-mobile.directAccessGrantsEnabled = true`. `otaway-api` no se toca.
...
- **ROPC solo para `otaway-mobile`, nunca para `otaway-api`** — el cliente web sigue con el estándar de la industria (Authorization Code + PKCE); el riesgo se acota al mínimo necesario, no se generaliza.
