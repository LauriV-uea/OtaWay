...
Permite que un administrador autorizado abra un rol organizacional existente desde `/admin/roles` en `otaway-ionic`, consulte sus permisos asociados y gestione esas asociaciones en una pantalla dedicada. La interfaz obtiene tanto el rol como el catálogo de permisos desde `otaway`, muestra los permisos agrupados por su módulo real, permite buscar, seleccionar y deseleccionar, presenta el resumen de altas y bajas pendientes y persiste el conjunto mediante un guardado explícito.
...
- [ ] `otaway-ionic/spec/constitution/api-integration.md` documenta el consumo dedicado antes de implementar la pantalla.
