...
vi.hoisted(() => {
	...
	process.env.DATABASE_URL = 'postgresql://user:password@localhost:5432/otaway?schema=public';
	...
	process.env.OAUTH_ISSUER = 'https://provider.example.com/realms/otaway';
	process.env.OAUTH_CLIENT_ID = 'client-id';
	process.env.OAUTH_CLIENT_SECRET = 'client-secret';
	process.env.OAUTH_REDIRECT_URI = 'http://localhost:3000/api/auth/callback';
	process.env.OAUTH_MOBILE_CLIENT_ID = 'otaway-mobile';
	process.env.OAUTH_ADMIN_CLIENT_ID = 'otaway-registration-service';
...
describe('register (servicio de registro público)', () => {
	...
	it('rechaza con 409 si el email ya existe en OtaWay, sin llamar a Keycloak', async () => {
	...
	it('traduce un conflicto de Keycloak a 409 sin crear nada en OtaWay', async () => {
	...
	it('revierte (elimina) el usuario de Keycloak si el paso de OtaWay falla', async () => {
	...
	it('revierte Keycloak si createUserWithAccessRequest falla (gestor-de-cancha) — bug real encontrado y corregido: antes esto dejaba un usuario huérfano en OtaWay', async () => {
