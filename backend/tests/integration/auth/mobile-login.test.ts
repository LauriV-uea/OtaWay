...
vi.hoisted(() => {
	...
	process.env.SESSION_COOKIE_NAME = 'otaway_session';
	process.env.SESSION_TEMP_COOKIE_NAME = 'otaway_oauth_state';
...
describe('auth mobile login route (ROPC, cliente móvil)', () => {
	...
	it('logs in with username/password using the public mobile client and returns a sessionToken', async () => {
		...
		await handler(buildRequest({ username: 'futbolista', password: 'otaway123' }), response);

		expect(passwordGrant).toHaveBeenCalledWith('futbolista', 'otaway123');
		expect(createSession).toHaveBeenCalledWith(
			'11111111-1111-1111-1111-111111111111',
			expect.objectContaining({ accessToken: 'access-token', clientId: 'otaway-mobile' }),
		);
