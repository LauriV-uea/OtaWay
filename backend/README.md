...
OtaWay es el backend REST de una plataforma SaaS multiempresa para gestionar organizaciones, sedes, usuarios, roles y permisos de espacios deportivos. El cliente oficial está en [`otaway-ionic`](../otaway-ionic/README.md).
...
```bash
git clone <URL-DEL-REPOSITORIO-OTAWAY>
cd otaway
```

Si ya recibió la carpeta, entre en ella. Los comandos siguientes se ejecutan desde `otaway/`.
...
```bash
createdb otaway
psql 'postgresql://USUARIO:CONTRASENA@localhost:5432/otaway?schema=public' -c 'select 1;'
```
...
docker compose up -d
docker compose ps
curl -I http://localhost:8081/realms/otaway
```

Compose inicia Keycloak 26, publica el puerto 8081 e importa `keycloak/realm-otaway.json`. Espere el estado saludable. El realm contiene clientes y cuentas didácticas: `futbolista`, `gestor` y `administrador`, con contraseña pública de laboratorio `otaway123`. Nunca reutilice esas credenciales ni despliegue el realm sin cambiarlas.
...
Una **semilla** (_seed_) introduce los datos mínimos que la aplicación necesita para funcionar. En OtaWay hay dos semillas diferentes y deben ejecutarse en este orden, después de las migraciones:
...
3. Si la base todavía no tiene organizaciones, crea `OtaWay Demo` con estado `ACTIVE`.
...
2. Inicie sesión una vez con `administrador` / `otaway123`, usando el frontend o `http://localhost:3000/api/auth/login`. Esto sincroniza `administrador@otaway.local` en PostgreSQL. Después cierre sesión.

3. En otra terminal, desde `otaway/`, asigne el rol global:

   ```bash
   yarn asignar-rol --email administrador@otaway.local --rol administrador
   ```
...
```bash
yarn asignar-rol --email futbolista@otaway.local --rol futbolista
yarn asignar-rol --email gestor@otaway.local --rol gestor-de-cancha
```
...
- Capacitor: `POST /api/auth/mobile/login` usa `otaway-mobile`, devuelve una sesión Bearer y el cliente la guarda en almacenamiento nativo seguro.
...
# Terminal 2, otaway-ionic/
yarn dev
```

En navegador, Vite reenvía `/api` desde 5173 hacia 3000, conservando el flujo de cookie. Android/iOS usan URL absoluta y Bearer. Consulte el [README móvil](../otaway-ionic/README.md) para emuladores y dispositivos.
